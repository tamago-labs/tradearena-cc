...
// Schema for check entitlement request
export const CheckEntitlementRequestSchema = z.object({
  paymentId: z.string().describe('Payment ID to check entitlement for'),
});

export type CheckEntitlementRequest = z.infer<typeof CheckEntitlementRequestSchema>;
...
export const cronos_x402_check_entitlement: McpTool = {
  ...
  schema: CheckEntitlementRequestSchema,
  
  async handler(agent: any, input: Record<string, any>): Promise<CallToolResult> {
    try {
      const { paymentId } = input as CheckEntitlementRequest;
